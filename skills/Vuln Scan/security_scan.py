...
SECRET_PATTERNS = [
    ...
    (r'(?:authorization|auth)\s*[=:]\s*["\']bearer\s+[a-zA-Z0-9\-_.]{10,}["\']', "Bearer Token", "critical"),
    ...
    (r'(?:database_url|db_url|dsn|connection_string)\s*[=:]\s*["\'](?:mongodb|postgres|mysql|redis):\/\/[^\s"\']+["\']', "Database Connection String", "critical"),
...
def _is_web_project(project_root: Path) -> bool:
    """Best-effort detection of whether security-header checks apply."""
    web_markers = (
        "package.json",
        "next.config.js",
        "next.config.mjs",
    ...
    if any((project_root / marker).exists() for marker in web_markers):
        return True

    # Python web frameworks
    framework_pattern = re.compile(
        r"from\s+fastapi\s+import|import\s+fastapi|FastAPI\s*\("
        r"|from\s+flask\s+import|import\s+flask|Flask\s*\("
        r"|from\s+django\s+import|import\s+django|DJANGO_SETTINGS_MODULE",
        re.IGNORECASE,
    )
    for py_file in project_root.rglob("*.py"):
        ...
        try:
            ...
            continue
        if framework_pattern.search(content):
            return True
    return False
...
def scan_configuration(project_path: str) -> Dict[str, Any]:
    ...
    # Check for security header configurations only for web projects.
    header_files = ["next.config.js", "next.config.mjs", "middleware.ts", "nginx.conf"]
    if _is_web_project(project_root):
        for hf in header_files:
            hf_path = project_root / hf
            if hf_path.exists():
        ...
        results["checks"]["security_headers_config"] = "not_applicable"
